...
expName = 'test_eyetracker'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Staff\\PsychoPy-experiments\\eye_tracking_engagement\\test_eyetracker_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win = visual.Window(
            ...
            monitor='testMonitor', color=[-1.0000, -0.6863, -0.6863], colorSpace='rgb',
        ...
        win.color = [-1.0000, -0.6863, -0.6863]
        win.colorSpace = 'rgb'
        win.backgroundImage = ''
...
def setupDevices(expInfo, thisExp, win):
    ...
    ioServer = io.launchHubServer(window=win, experiment_code='test_eyetracker', session_code=ioSession, datastore_name=thisExp.dataFileName, **ioConfig)
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    slider_granularity=.05
    nTicks = int(1 / slider_granularity) + 1
    #Failing to set ticks or labels in code
    slider_ticks=[
          slider_granularity*i for i in range(nTicks)
        ]
    slider_labels='0', '1'
    ...
    video = visual.MovieStim(
        win, name='video',
        filename='Downloads/eye_tracker/test`/output.mp4', movieLib='ffpyplayer',
        loop=False, volume=1.0, noAudio=True,
        pos=(0, 0), size=(1.77777777778, 1), units=win.units,
    ...
    slider = visual.Slider(win=win, name='slider',
        startValue=slider_start, size=(slider_width, slider_height), pos=(0, -0.4), units=win.units,
        ...
        flip=False, ori=slider_orientation, depth=-2, readOnly=False)
    ...
    startComponents = [text, key_resp, etRecord_2, mouse]
    ...
    while continueRoutine:
        ...
        if etRecord_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'etRecord_2.started')
            # update status
        ...
        if etRecord_2.status == STARTED:
            ...
            if tThisFlipGlobal > etRecord_2.tStartRefresh + 0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'etRecord_2.stopped')
                # update status
    ...
    while continueRoutine:
        ...
        if video.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > video.tStartRefresh + 60.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        #if frame_n % skip_steps == 0:
        sliderDiff = mouse.getPos()[slider_orientation] - mouseRec[slider_orientation]
        #slider.markerPos += sliderDiff
        #slidingSteps = sliderDiff * sliding_factor / slider_granularity
        #print(sliderDiff, sliderDiff * sliding_factor, slidingSteps)
        #slider.markerPos += slidingSteps * slider_granularity
        if sliderDiff > 0.001:
            slider.markerPos += slider_granularity
        elif sliderDiff < -0.001:
            slider.markerPos -= slider_granularity
        #print(sliderDiff, slider.markerPos)
        
        if slider.markerPos:
            slider_data.append(slider.markerPos)
            slider_times.append(t)
    ...
    thisExp.addData('slider.responsesVal', slider_data)
    thisExp.addData('slider.responsesT', slider_times)
    print('slider.responsesVal')
    print('slider.responsesVal', slider_data)
    print('slider.responsesT', slider_times)
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    endComponents = [etRecord, text_2, key_resp_2]
    ...
    while continueRoutine:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['y','n','left','right','space', 'enter'], ignoreKeys=["escape"], waitRelease=False)
